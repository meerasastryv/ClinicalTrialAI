...
class PerformanceReport:
    ...
    def generate_report(self, top_n=10):
        ...
        for metric in metrics:
            ...
            lines.append(f"Average Time  : {metric.average_time:.6f} ms")
            lines.append(f"Minimum Time  : {metric.min_time:.6f} ms")
            lines.append(f"Maximum Time  : {metric.maximum_time:.6f} ms")
            lines.append(f"Total Time    : {metric.total_time:.6f} ms")
            lines.append("-" * 80)
        lines.append("")
        lines.append("=" * 80)
        ...
        for index, metric in enumerate(metrics[:top_n], start=1):
            ...
            lines.append(
                f"    Avg={metric.average_time:.6f} ms | "
                f"Max={metric.maximum_time:.6f} ms | "
